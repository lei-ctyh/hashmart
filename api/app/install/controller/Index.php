...
class Index extends BaseController
{
    ...
    public function install()
    {
        if (request()->isAjax()) {
            ...
            // 替换数据库表前缀
            $sql = explode(";\n", $sql);
            $sql = str_replace(" `hashmart_", " `{$mysql['prefix']}", $sql);
            $sql = str_replace("HASHMART_URL", $domain, $sql);
            ...
            mysqli_query($connect, "UPDATE {$mysql['prefix']}sys_admin SET password='{$pwd}', salt='{$salt}' where id = 1");

            mysqli_query($connect, "UPDATE {$mysql['prefix']}sys_setting SET value='{$domain}' where `key` = 'api_url'");

            $configJs =<<<EOL
const APP_CONFIG = {
	APP_NAME: "HashMart",
	API_URL: "{$domain}/admin"
}
EOL;
            file_put_contents(root_path() . '/public/backend/config.js', $configJs);
    ...
    {
        if (request()->isAjax()
            && is_file('../extend/install/install.lock')) {
            ...
            try {
                ...
                if (file_put_contents(public_path() . uniqid() . '_install_back' . '.php', $content)) {
                    unlink($install);
                }
            } catch (\Throwable $th) {
                echo $th->getMessage();
